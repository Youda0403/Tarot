...
(async()=>{
 ...
 await page.getByRole('button',{name:'이번 판 실패'}).click();
 assert.equal(await page.getByRole('button',{name:'이번 판 실패'}).getAttribute('aria-pressed'),'true');
 await page.getByRole('button',{name:'사진 2 뽑기'}).click();
 assert.equal(await page.getByRole('button',{name:'사진 2 뽑기'}).getAttribute('aria-pressed'),'true');
 ...
 assert.equal(await page.getByLabel('캐릭터 2 사진 선택').count(),0);
